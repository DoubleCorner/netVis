function AreaChart() {
  ...
  function createChart() {
    ...
    areaChart.padding_width =
      areaChart.width - areaChart.padding.left - areaChart.padding.right;
    areaChart.padding_height =
      areaChart.height - areaChart.padding.bottom - areaChart.padding.top;
    areaChart.x_scale = d3.time
      .scale()
      .domain(
        d3.extent(init_data_line, function(d) {
          return d.date;
        })
      )
      .range([0, areaChart.padding_width]);
    areaChart.y_scale = d3.scale
      .linear()
      .domain(
        d3.extent(init_data_line, function(d) {
          return d.value;
    ...
    areaChart.x_axis = d3.svg
      .axis()
      .scale(areaChart.x_scale)
      .orient('bottom');

    areaChart.zoom = d3.behavior
      .zoom()
      .x(areaChart.x_scale)
    ...
    areaChart.svg
      ...
      .attr(
        'transform',
        'translate(' +
          areaChart.padding.left +
          ',' +
          (areaChart.height - areaChart.padding.bottom) +
          ')'
      )
      .call(areaChart.x_axis);

    areaChart.area_g = areaChart.svg
      .append('g')
      .attr(
        'transform',
        'translate(' +
