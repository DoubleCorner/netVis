function BackChart() {
  ...
  function resizeFull() {
    ...
    mainChart.nodes_label
      ...
      .attr('transform', function(d) {
        return (
          'rotate(30,' +
          mainChart.x_scale(d.x) +
          ' ' +
          mainChart.y_scale(d.y) +
          ')'
        );
      });
      ...
      .on('mousemove', function() {
        ...
        if (!s.empty() && mainChart.move_state === 0) {
          ...
          mainChart.svg_nodes.each(function(every) {
            ...
            var node_x =
              mainChart.scale * parseFloat(true_node_x) +
              mainChart.translate[0];
            var node_y =
              mainChart.scale * parseFloat(true_node_y) +
  ...
  function handleData(view_data) {
    ...
    mainChart.x_scale = d3.scale
      .linear()
      .domain(
        d3.extent(mainChart.nodes, function(d) {
          return d.x;
        })
      )
      .range([padding.left, mainChart.width - padding.right]);

    mainChart.y_scale = d3.scale
      .linear()
      .domain(
        d3.extent(mainChart.nodes, function(d) {
          return d.y;
  ...
  function level(level) {
    if (SHOW_ALL) {
      ...
      mainChart.svg_links.attr('display', function(d) {
        ...
        return parseInt(source_level) > log || parseInt(target_level) > log
          ? 'none'
          : 'block';
      });
    }
        ...
        mainChart.nodes.forEach(function(item) {
          if (item.id === t.target) {
            ...
            if (!mainChart.label_show_flag) {
              ...
              d3.select('#node_' + t.target + ' line').attr(
                'visibility',
                'hidden'
              );
            }
        ...
        mainChart.nodes.forEach(function(item) {
          if (item.id === t.source) {
            ...
            if (!mainChart.label_show_flag) {
              ...
              d3.select('#node_' + t.source + ' line').attr(
                'visibility',
                'hidden'
              );
            }
