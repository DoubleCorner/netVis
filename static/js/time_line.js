function TimeLineChart() {
  ...
  function init() {
    ...
    timeChart.button_play = timeChart.tools
      .append('button')
      .attr({
        type: 'button',
  ...
  function createErase() {
    timeChart.button_earse = timeChart.tools
      .append('button')
      .attr({
        type: 'button',
        class: 'btn btn-default'
  ...
  function createChart() {
    ...
    timeChart.padding_width =
      timeChart.width - timeChart.padding.left - timeChart.padding.right;
    timeChart.padding_height =
      timeChart.height - timeChart.padding.top - timeChart.padding.bottom;
    timeChart.svg = d3
      .select('#time_line')
      .append('svg')
    ...
    /***创建轴线***/
    //x轴线
    timeChart.x_axis = d3.svg
      .axis()
      .scale(timeChart.x_scale)
      .orient('bottom')
      .ticks(d3.time.minutes, 5);
    //y方向上的轴线
    timeChart.y_axis = d3.svg
      .axis()
    ...
    timeChart.rects = timeChart.svg
      ...
      .attr(
        'transform',
        'translate(' +
          timeChart.padding.left +
          ',' +
          timeChart.padding.top +
          ')'
      )
    ...
    timeChart.svg
      ...
      .attr(
        ...
        'translate(' +
          timeChart.padding.left +
          ',' +
          (timeChart.height - timeChart.padding.bottom) +
          ')'
      )
  ...
  function play() {
    removeBrush();
    timeChart.timer = setInterval(
      function() {
        var maxData = 1;
        for (
          var j = now_time_index;
          j <= now_time_index + timeChart.numbers;
          j++
        ) {
        ...
        //新的比例尺
        timeChart.x_scale.domain([
          init_data_line[now_time_index].date,
          init_data_line[timeChart.numbers + now_time_index].date
        ]);
        ...
        updateMain([
          init_data_line[now_time_index].date,
          init_data_line[now_time_index + 1].date
        ]);
      },
      REFRESH_FREQUENCY,
      90
  ...
  function brushEnd() {
    ...
    extent[0].setMinutes(
      extent[0].getMinutes() - (extent[0].getMinutes() % TIME_INTERVAL)
    );
    extent[1].setMinutes(
      extent[1].getMinutes() - (extent[1].getMinutes() % TIME_INTERVAL)
    );
    ...
    if (
      extent[0].getTime() !== timeChart.brush_extent[0].getTime() ||
      extent[1].getTime() !== timeChart.brush_extent[1].getTime()
    ) {
      ...
      var packages =
        (timeChart.brush_extent[1].getTime() -
          timeChart.brush_extent[0].getTime()) /
        (1000 * 60 * TIME_INTERVAL);
      if (packages > MAX_PACKAGES) {
        alert('请选择小于等于10的包数！');
        clearBrush();
        return false;
      } else if (!timeChart.brush.empty()) updateMain(timeChart.brush_extent);
    }
  }
  ...
  function createBrush() {
    timeChart.brush = d3.svg
      .brush()
      .x(timeChart.x_scale)
      .on('brushend', brushEnd);
    timeChart.g_brush = timeChart.svg
      .append('g')
      .attr('class', 'brush')
      .attr(
        'transform',
