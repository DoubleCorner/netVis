function ForceChart() {
  ...
  function init() {
    ...
    mainChart.distance = mainChart.parameters
      .append('div')
      .attr('class', 'rows');
    mainChart.distance
      .append('span')
  ...
  function resizeFull() {
    ...
    mainChart.g.attr(
      'transform',
      'translate(' + mainChart.translate + ')scale(' + mainChart.scale + ')'
    );
      ...
      .on('mousemove', function() {
        ...
        if (!s.empty() && mainChart.move_state === 0) {
          ...
          mainChart.svg_nodes.each(function(every) {
            var node_x =
              mainChart.scale * parseFloat(d3.select(this).attr('cx')) +
              mainChart.translate[0];
            var node_y =
              mainChart.scale * parseFloat(d3.select(this).attr('cy')) +
            ...
            if (
              ...
              mainChart.links.forEach(function(item) {
                if (item.source.id === every.id) {
                  ...
                  d3.select('#node_' + item.target.id + ' circle').attr(
                    'opacity',
                    REGION_OPACITY
                  );
                ...
                if (item.target.id === every.id) {
                  ...
                  d3.select('#node_' + item.source.id + ' circle').attr(
                    'opacity',
                    REGION_OPACITY
                  );
  ...
  function zoomed() {
    ...
    if (mainChart.scale > 1) {
      ...
      mainChart.nodes_line_label.attr(
        'stroke-width',
        INIT_NODE_LABEL_LINK_WIDTH / mainChart.scale
      );
    }
    mainChart.map_frame
      .attr(
  ...
  function drawGraph() {
    ...
    //节点可拖动
    mainChart.drag = d3.behavior.drag().on('drag', function(d) {
    ...
    //标签可拖动
    mainChart.label_drag = d3.behavior.drag().on('drag', function(d) {
      var x = d3.mouse(this)[0];
      var y = d3.mouse(this)[1];
      d3.select(this)
        .attr('x', x)
    ...
    mainChart.nodes_line_label = mainChart.svg_nodes_g
      .append('line')
    ...
    mainChart.map_drag = d3.behavior
      ...
      .on('drag', function() {
        ...
        mainChart.g.attr(
          'transform',
          'translate(' + translate + ')scale(' + mainChart.scale + ')'
        );
      });
  ...
  function level(level) {
    ...
    if (SHOW_ALL) {
      ...
      mainChart.svg_links.attr('display', function(d) {
        return parseInt(d.target.level) > log || parseInt(d.source.level) > log
          ? 'none'
          : 'block';
      });
    }
  ...
  function linkMoveOut(d) {
    ...
    mainChart.nodes.forEach(function(item) {
      if (item.id === d.source_name) {
        d3.select('#node_' + d.source_name + ' circle').attr(
          'fill',
          item.color
        );
        if (!mainChart.label_show_flag) {
          d3.select('#node_' + d.source_name + ' text').attr(
            'visibility',
            'hidden'
          );
          d3.select('#node_' + d.source_name + ' line').attr(
            'visibility',
            'hidden'
          );
        }
      ...
      if (item.id === d.target_name) {
        d3.select('#node_' + d.target_name + ' circle').attr(
          'fill',
          item.color
        );
        if (!mainChart.label_show_flag) {
          d3.select('#node_' + d.target_name + ' text').attr(
            'visibility',
            'hidden'
          );
          d3.select('#node_' + d.target_name + ' line').attr(
            'visibility',
            'hidden'
          );
        }
  ...
  function nodeMoveOver(d) {
    ...
    mainChart.links.forEach(function(t) {
      if (t.source.id === d.id) {
        ...
        d3.select('#node_' + t.target.id + ' circle').attr(
          'fill',
          TARGET_COLOR
        );
        d3.select('#node_' + t.target.id + ' text').attr(
          'visibility',
          'visible'
        );
        d3.select('#node_' + t.target.id + ' line').attr(
          'visibility',
          'visible'
        );
      } else if (t.target.id === d.id) {
        d3.select('#link_' + t.id).attr('stroke', SOURCE_COLOR);
        d3.select('#node_' + t.source.id + ' circle').attr(
          'fill',
          SOURCE_COLOR
        );
        d3.select('#node_' + t.source.id + ' text').attr(
          'visibility',
          'visible'
        );
        d3.select('#node_' + t.source.id + ' line').attr(
          'visibility',
          'visible'
        );
      }
      ...
      if (t.source.id === d.id) {
        ...
        mainChart.nodes.forEach(function(item) {
          if (item.id === t.target.id) {
            ...
            if (
              d3
                .select('#node_' + t.target.id + ' circle')
                .classed('select_node')
            )
              d3.select('#node_' + t.target.id + ' circle').attr(
                'fill',
        ...
        mainChart.nodes.forEach(function(item) {
          if (item.id === t.source.id) {
            ...
            if (
              d3
                .select('#node_' + t.source.id + ' circle')
                .classed('select_node')
            )
              d3.select('#node_' + t.source.id + ' circle').attr(
                'fill',
