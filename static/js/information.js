function InfChart() {
  ...
  var x_scale = d3.scale
    .linear()
    .domain([0, step_number])
    .range([0, padding_width]);
  var item_list = {};
          ...
          item_div
            ...
            .on('click', function() {
              if (!item_list.degree.brush.empty()) {
                item_list.degree.brush.clear();
                item_list.degree.brush(
                  d3.select('.brush.' + 'info_degree').transition()
                );
                item_list.degree.brush.event(
                  d3
                    .select('.brush.' + 'info_degree')
                    .transition()
                    .delay(500)
                );
              }
      ...
      case '度中心性分布':
        if (!item_list.degree_centrality.flag) {
          item_div = list_container
            .append('div')
            .attr('class', 'item_div')
            .attr('id', 'info_degree_centrality');
          item_div
            .append('img')
            .attr('class', 'info_close')
            .attr('src', '../static/img/close.png')
          ...
          item_div
            ...
            .on('click', function() {
              if (!item_list.degree_centrality.brush.empty()) {
                item_list.degree_centrality.brush.clear();
                item_list.degree_centrality.brush(
                  d3.select('.brush.' + 'info_degree_centrality').transition()
                );
                item_list.degree_centrality.brush.event(
                  d3
                    .select('.brush.' + 'info_degree_centrality')
                    .transition()
                    .delay(500)
                );
              }
          ...
          drawOneGraph(
            item_list.degree_centrality.data,
            'info_degree_centrality',
            '度中心性'
          );
        }
        break;
      case '接近中心性分布':
        if (!item_list.closeness_centrality.flag) {
          item_div = list_container
            .append('div')
            .attr('class', 'item_div')
            .attr('id', 'info_closeness_centrality');
          item_div
            .append('img')
            .attr('class', 'info_close')
            .attr('src', '../static/img/close.png')
          ...
          item_div
            ...
            .on('click', function() {
              if (!item_list.closeness_centrality.brush.empty()) {
                item_list.closeness_centrality.brush.clear();
                item_list.closeness_centrality.brush(
                  d3
                    .select('.brush.' + 'info_closeness_centrality')
                    .transition()
                );
                item_list.closeness_centrality.brush.event(
                  d3
                    .select('.brush.' + 'info_closeness_centrality')
                    .transition()
                    .delay(500)
          ...
          drawOneGraph(
            item_list.closeness_centrality.data,
            'info_closeness_centrality',
            '接近中心性'
          );
        }
        break;
      case '介数中心性分布':
        if (!item_list.betweness_centrality.flag) {
          item_div = list_container
            .append('div')
            .attr('class', 'item_div')
            .attr('id', 'info_betweness_centrality');
          item_div
            .append('img')
            .attr('class', 'info_close')
            .attr('src', '../static/img/close.png')
          ...
          item_div
            ...
            .on('click', function() {
              if (!item_list.betweness_centrality.brush.empty()) {
                item_list.betweness_centrality.brush.clear();
                item_list.betweness_centrality.brush(
                  d3
                    .select('.brush.' + 'info_betweness_centrality')
                    .transition()
                );
                item_list.betweness_centrality.brush.event(
                  d3
                    .select('.brush.' + 'info_betweness_centrality')
                    .transition()
                    .delay(500)
          ...
          drawOneGraph(
            item_list.betweness_centrality.data,
            'info_betweness_centrality',
            '介数中心性'
          );
        }
        break;
      case '特征向量中心性分布':
        if (!item_list.eigenvector_centrality.flag) {
          item_div = list_container
            .append('div')
            .attr('class', 'item_div')
            .attr('id', 'info_eigenvector_centrality');
          item_div
            .append('img')
            .attr('class', 'info_close')
            .attr('src', '../static/img/close.png')
          ...
          item_div
            ...
            .on('click', function() {
              if (!item_list.eigenvector_centrality.brush.empty()) {
                item_list.eigenvector_centrality.brush.clear();
                item_list.eigenvector_centrality.brush(
                  d3
                    .select('.brush.' + 'info_eigenvector_centrality')
                    .transition()
                );
                item_list.eigenvector_centrality.brush.event(
                  d3
                    .select('.brush.' + 'info_eigenvector_centrality')
                    .transition()
                    .delay(500)
          ...
          drawOneGraph(
            item_list.eigenvector_centrality.data,
            'info_eigenvector_centrality',
            '特征向量中心性'
          );
        }
        break;
          ...
          item_div
            ...
            .on('click', function() {
              if (!item_list.clustering.brush.empty()) {
                item_list.clustering.brush.clear();
                item_list.clustering.brush(
                  d3.select('.brush.' + 'info_clustering').transition()
                );
                item_list.clustering.brush.event(
                  d3
                    .select('.brush.' + 'info_clustering')
                    .transition()
                    .delay(500)
                );
              }
          ...
          item_div
            ...
            .on('click', function() {
              if (!item_list.port.brush.empty()) {
                item_list.port.brush.clear();
                item_list.port.brush(
                  d3.select('.brush.' + 'info_port').transition()
                );
                item_list.port.brush.event(
                  d3
                    .select('.brush.' + 'info_port')
                    .transition()
                    .delay(500)
                );
              }
          ...
          item_div
            ...
            .on('click', function() {
              if (!item_list.continuous.brush.empty()) {
                item_list.continuous.brush.clear();
                item_list.continuous.brush(
                  d3.select('.brush.' + 'info_continuous').transition()
                );
                item_list.continuous.brush.event(
                  d3
                    .select('.brush.' + 'info_continuous')
                    .transition()
                    .delay(500)
                );
              }
          ...
          item_div
            ...
            .on('click', function() {
              if (!item_list.discrete.brush.empty()) {
                item_list.discrete.brush.clear();
                item_list.discrete.brush(
                  d3.select('.brush.' + 'info_discrete').transition()
                );
                item_list.discrete.brush.event(
                  d3
                    .select('.brush.' + 'info_discrete')
                    .transition()
                    .delay(500)
                );
              }
    ...
    function brushEnd() {
      ...
      if (brush_extent[0] !== extent[0] || brush_extent[1] !== extent[1]) {
        ...
        if (!now_brush.empty()) {
          var true_range = [
            his_data[extent[0]].x,
            his_data[extent[0]].x + (extent[1] - extent[0]) * his_data[0].dx
          ];
          var error_range = [
            true_range[0] - his_data[0].dx / 1000,
            true_range[1] + his_data[0].dx / 1000
          ]; //刚好相等时候由于很小的错误导致无法筛选
          switch (div_id) {
            case 'info_degree':
              item_list.degree.select_data = [];
              graph.parameters.forEach(function(value) {
                if (
                  value.degree >= error_range[0] &&
                  value.degree <= error_range[1]
                ) {
              ...
              tip_label.text(
                '数量：' +
                  item_list.degree.select_data.length +
                  ' 范围：' +
                  Math.ceil(true_range[0]) +
                  ' ~ ' +
                  Math.floor(true_range[1])
              );
              break;
            case 'info_degree_centrality':
              item_list.degree_centrality.select_data = [];
              graph.parameters.forEach(function(value) {
                if (
                  value.degree_centrality >= error_range[0] &&
                  value.degree_centrality <= error_range[1]
                ) {
              ...
              tip_label.text(
                '数量：' +
                  item_list.degree_centrality.select_data.length +
                  ' 范围：' +
                  true_range[0].toExponential(2) +
                  ' ~ ' +
                  true_range[1].toExponential(2)
              );
              break;
            case 'info_closeness_centrality':
              item_list.closeness_centrality.select_data = [];
              graph.parameters.forEach(function(value) {
                if (
                  value.closeness_centrality >= error_range[0] &&
                  value.closeness_centrality <= error_range[1]
                ) {
              ...
              tip_label.text(
                '数量：' +
                  item_list.closeness_centrality.select_data.length +
                  ' 范围：' +
                  true_range[0].toExponential(2) +
                  ' ~ ' +
                  true_range[1].toExponential(2)
            ...
            case 'info_betweness_centrality':
              ...
              graph.parameters.forEach(function(value) {
                if (
                  value.betweness_centrality >= error_range[0] &&
                  value.betweness_centrality <= error_range[1]
                ) {
              ...
              tip_label.text(
                '数量：' +
                  item_list.betweness_centrality.select_data.length +
                  ' 范围：' +
                  true_range[0].toExponential(2) +
                  ' ~ ' +
                  true_range[1].toExponential(2)
            ...
            case 'info_eigenvector_centrality':
              ...
              graph.parameters.forEach(function(value) {
                if (
                  value.eigenvector_centrality >= error_range[0] &&
                  value.eigenvector_centrality <= error_range[1]
                ) {
              ...
              tip_label.text(
                '数量：' +
                  item_list.eigenvector_centrality.select_data.length +
                  ' 范围：' +
                  true_range[0].toExponential(2) +
                  ' ~ ' +
                  true_range[1].toExponential(2)
            ...
            case 'info_clustering':
              ...
              graph.parameters.forEach(function(value) {
                if (
                  value.clustering >= error_range[0] &&
                  value.clustering <= error_range[1]
                ) {
              ...
              tip_label.text(
                '数量：' +
                  item_list.clustering.select_data.length +
                  ' 范围：' +
                  true_range[0].toExponential(2) +
                  ' ~ ' +
                  true_range[1].toExponential(2)
            ...
            case 'info_port':
              ...
              graph.parameters.forEach(function(value) {
                if (
                  value.port >= error_range[0] &&
                  value.port <= error_range[1]
                ) {
              ...
              tip_label.text(
                '数量：' +
                  item_list.port.select_data.length +
                  ' 范围：' +
                  Math.ceil(true_range[0]) +
                  ' ~ ' +
                  Math.floor(true_range[1])
            ...
            case 'info_continuous':
              ...
              graph.parameters.forEach(function(value) {
                if (
                  value.continuous >= error_range[0] &&
                  value.continuous <= error_range[1]
                ) {
              ...
              tip_label.text(
                '数量：' +
                  item_list.continuous.select_data.length +
                  ' 范围：' +
                  Math.ceil(true_range[0]) +
                  ' ~ ' +
                  Math.floor(true_range[1])
            ...
            case 'info_discrete':
              ...
              graph.parameters.forEach(function(value) {
                if (
                  value.discrete >= error_range[0] &&
                  value.discrete <= error_range[1]
                ) {
              ...
              tip_label.text(
                '数量：' +
                  item_list.discrete.select_data.length +
                  ' 范围：' +
                  Math.ceil(true_range[0]) +
                  ' ~ ' +
                  Math.floor(true_range[1])
