function BundleChart() {
  ...
  function init() {
    ...
    mainChart.parameters = d3
      .select('#main')
      .append('div')
      .attr('class', 'parameters');

    mainChart.tension = mainChart.parameters
      .append('div')
      .attr('class', 'rows');
    mainChart.tension
      .append('span')
    ...
    mainChart.tension
      ...
      .on('input', function() {
        d3.select(this).style(
          'background-size',
          ((this.value - this.min) / (this.max - this.min)) * 100 + '% 100%'
        );
        ...
        mainChart.result_links = mainChart.bundle(
          reflection(mainChart.result_nodes, mainChart.links)
        );
        mainChart.svg_links.attr('d', mainChart.line);

        mainChart.cluster.size([360, mainChart.mini_inner_radius]);
        mainChart.mini_result_nodes = mainChart.cluster.nodes(
          mainChart.nodes_trans
        );
        mainChart.mini_result_links = mainChart.bundle(
          reflection(mainChart.mini_result_nodes, mainChart.links)
        );
  ...
  function resizeFull() {
    ...
    mainChart.g.style(
      'transform',
      'translate(' +
        mainChart.translate[0] +
        'px,' +
        mainChart.translate[1] +
        'px)scale(' +
        mainChart.scale +
        ')'
    );
    ...
    mainChart.svg_links
      .attr('stroke-opacity', function(d) {
        for (var index = 0; index !== mainChart.links.length; index++) {
          if (
            d[0].id === mainChart.links[index].source &&
            d[2].id === mainChart.links[index].target
          ) {
  ...
  function regionSelect() {
    ...
    mainChart.svg
      .on('mousedown', function() {
        ...
        mainChart.svg.append('rect').attr({
      ...
      .on('mousemove', function() {
        ...
        if (!s.empty() && mainChart.move_state === 0) {
          ...
          mainChart.svg_nodes.each(function(every) {
            ...
            var node_x =
              mainChart.scale * (cx + mainChart.width / 2) +
              mainChart.translate[0];
            var node_y =
              mainChart.scale * (cy + mainChart.height / 2) +
              mainChart.translate[1];
            if (
              node_x >= parameters.x &&
              node_x <= parameters.x + parameters.width &&
              node_y >= parameters.y &&
              node_y <= parameters.y + parameters.height
            ) {
              ...
              mainChart.links.forEach(function(item) {
                if (item.source === every.id) {
                  d3.select('#link_' + item.id).attr(
                    'stroke-opacity',
                    REGION_OPACITY
                  );
                  d3.select('#node_' + item.target + ' circle').attr(
                    'opacity',
                    REGION_OPACITY
                  );
                ...
                if (item.target === every.id) {
                  ...
                  d3.select('#node_' + item.source + ' circle').attr(
                    'opacity',
                    REGION_OPACITY
                  );
  ...
  function zoomed() {
    ...
    if (mainChart.scale > 1) {
      ...
      mainChart.nodes_label.attr(
        'font-size',
        mainChart.now_label_size / mainChart.scale
      );
    }
    mainChart.map_frame
      .attr(
        'transform',
        'translate(' +
          (-mainChart.translate[0] * mainChart.mini_scale) / mainChart.scale +
          ',' +
  ...
  function drawGraph() {
    ...
    mainChart.group = mainChart.svg
      .append('g')
      .attr(
        'transform',
        'translate(' + mainChart.width / 2 + ',' + mainChart.height / 2 + ')'
      );

    /*mainChart.group的变换导致mainChart.g的坐标原点并不在左上角，而在svg的坐标变换中不能设置变换的相对坐标，但是css3可以设置相对坐标（attr使用svg变换，style使用css3变换）*/
    mainChart.g = mainChart.group
      .append('g')
      .style(
        'transform-origin',
        -mainChart.width / 2 + 'px ' + -mainChart.height / 2 + 'px'
      );
    // console.log('--------------------')
    // console.log(mainChart.result_links);
    mainChart.svg_links = mainChart.g
      .selectAll('.links')
    ...
    mainChart.selected_link
      .attr('stroke', CLICK_SELECT_COLOR)
      .classed('select_link', true);
    control_chart.updateLink(mainChart.selected_link_data);

    mainChart.svg_nodes_g = mainChart.g
      .selectAll('.nodes')
      .data(
        mainChart.result_nodes.filter(function(d) {
          return !d.children;
        })
      )
      .enter()
    ...
    mainChart.svg_nodes = mainChart.svg_nodes_g
      .append('circle')
      .attr('r', function(d) {
        return +d.size;
      })
    ...
    mainChart.selected_node = d3.select(
      '#node_' + mainChart.nodes[0].id + ' circle'
    );
    mainChart.selected_node_data = mainChart.nodes[0];
    mainChart.selected_node
      .attr('fill', CLICK_SELECT_COLOR)
      .classed('select_node', true);
    control_chart.updateNode(mainChart.selected_node_data);

    mainChart.nodes_label = mainChart.svg_nodes_g
      .append('text')
    ...
    mainChart.mini_height =
      mainChart.mini_width * (mainChart.height / mainChart.width);
    mainChart.mini_scale = mainChart.mini_width / mainChart.width;

    mainChart.map_svg = d3
      .select('#mini_map')
      .append('svg')
      .attr('width', mainChart.mini_width)
      .attr('height', mainChart.mini_height)
      .attr(
        'transform',
        'translate(0,' +
          (mainChart.mini_width - mainChart.mini_height) / 2 +
          ')'
      );
  ...
  function level(level) {
    if (SHOW_ALL) {
      ...
      mainChart.svg_links.attr('display', function(d) {
        if (d.length == 1) return 'none';
        return parseInt(d[0].level) > log || parseInt(d[2].level) > log
          ? 'none'
          : 'block';
      });
    }
  ...
  function miniMap() {
    mainChart.mini_inner_radius =
      mainChart.mini_height / 2 - mainChart.padding * mainChart.mini_scale;
    mainChart.cluster.size([360, mainChart.mini_inner_radius]);
    mainChart.mini_result_nodes = mainChart.cluster.nodes(
      mainChart.nodes_trans
    );
    ...
    mainChart.map_group = mainChart.map_svg
      .append('g')
      .attr(
        'transform',
        'translate(' +
          mainChart.mini_width / 2 +
          ',' +
          mainChart.mini_height / 2 +
          ')'
      );
    mainChart.map_g = mainChart.map_group.append('g');
    mainChart.mini_svg_links = mainChart.map_g
      .selectAll('.m_links')
    ...
    mainChart.map_g
      .selectAll('.m_nodes')
      .data(
        mainChart.mini_result_nodes.filter(function(d) {
          return !d.children;
        })
      )
      .enter()
    ...
    mainChart.map_drag = d3.behavior
      .drag()
      .on('dragstart', function() {
        mainChart.mini_translate = d3.transform(
          mainChart.map_frame.attr('transform')
        ).translate;
      })
      .on('drag', function() {
        mainChart.mini_translate[0] += d3.event.dx;
        mainChart.mini_translate[1] += d3.event.dy;
        mainChart.map_frame.attr(
          'transform',
          'translate(' +
            mainChart.mini_translate[0] +
            ',' +
            mainChart.mini_translate[1] +
            ')'
        );
        var translate = [
          (-mainChart.mini_translate[0] / mainChart.mini_scale) *
            mainChart.scale,
          (-mainChart.mini_translate[1] / mainChart.mini_scale) *
            mainChart.scale
        ];
        mainChart.g.style(
          'transform',
    ...
    mainChart.map_frame = mainChart.map_svg
      .append('rect')
  ...
  function linkClick(d) {
    if (d3.select(this).classed('select_link')) return false;
    else {
      mainChart.selected_link
        .attr('stroke', mainChart.selected_link_data.color)
        .classed('select_link', false);
  ...
  function linkMoveOut(d) {
    ...
    mainChart.nodes.forEach(function(item) {
      if (item.id === d[0]['id']) {
        ...
        d3.select('#node_' + d[0]['id'] + ' text').attr(
          'fill',
          mainChart.now_label_color
        );
      ...
      if (item.id === d[2]['id']) {
        ...
        d3.select('#node_' + d[2]['id'] + ' text').attr(
          'fill',
          mainChart.now_label_color
        );
        if (!mainChart.label_show_flag) {
  ...
  function nodeClick(d) {
    if (d3.select(this).classed('select_node')) return false;
    else {
      mainChart.selected_node
        .attr('fill', mainChart.selected_node_data.color)
        .classed('select_node', false);
  ...
  function nodeMoveOut(d) {
    ...
    if (d3.select(this).classed('select_node'))
      d3.select(this).attr('fill', CLICK_SELECT_COLOR);
    d3.select('#node_' + d.id + ' text').attr(
      'fill',
      mainChart.now_label_color
    );
    if (!mainChart.label_show_flag) {
      ...
      if (t.source === d.id) {
        ...
        mainChart.nodes.forEach(function(item) {
          if (item.id === t.target) {
            ...
            if (
              d3.select('#node_' + t.target + ' circle').classed('select_node')
            )
              d3.select('#node_' + t.target + ' circle').attr(
                'fill',
                CLICK_SELECT_COLOR
              );
            d3.select('#node_' + t.target + ' text').attr(
              'fill',
              mainChart.now_label_color
            );
            if (!mainChart.label_show_flag) {
        ...
        mainChart.nodes.forEach(function(item) {
          if (item.id === t.source) {
            ...
            if (
              d3.select('#node_' + t.source + ' circle').classed('select_node')
            )
              d3.select('#node_' + t.source + ' circle').attr(
                'fill',
                CLICK_SELECT_COLOR
              );
            d3.select('#node_' + t.source + ' text').attr(
              'fill',
              mainChart.now_label_color
            );
            if (!mainChart.label_show_flag) {
  ...
  BundleChart.prototype.setEdgeWidth = function(link_width) {
    if (EDGE_ALL) {
      ...
      mainChart.selected_link.attr(
        'stroke-width',
        link_width / mainChart.scale
      );
